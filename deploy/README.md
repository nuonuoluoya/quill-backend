...
API / PostgreSQL 使用独立命名卷，不自动加载本机 `.data` 中的 PGlite 数据。`docker compose down` 保留卷。不要在有需保留数据时使用 `down -v`。生产部署顺序参见 [独立 PostgreSQL 部署流程](PRODUCTION-PLAN.md)；专用生产配置尚待实现，不能直接使用本开发示例上线。
